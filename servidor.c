...
void serverTCP(int s, struct sockaddr_in clientaddr_in) {
    ...
    if (status) {
        /* Si no se pudo obtener el nombre, usamos la dirección IP */
    ...
    time(&timevar);
    printf("Conexión desde %s:%u a las %s", 
           hostname, ntohs(clientaddr_in.sin_port), ctime(&timevar));

    /* Configuración para un cierre limpio */
    linger.l_onoff = 1;
    linger.l_linger = 1;
    ...
    while ((len = recv(s, buf, TAM_BUFFER, 0)) > 0) {
        buf[len] = '\0'; /* Aseguramos que el buffer sea una cadena terminada en NULL */
        reqcnt++;        /* Incrementamos el contador de solicitudes */
    ...
    /* Si `recv` retorna 0, significa que el cliente cerró la conexión */
    if (len == 0) {
        printf("Cliente %s:%u cerró la conexión.\n", hostname, ntohs(clientaddr_in.sin_port));
    } else if (len < 0) {
        perror("recv");
    }
